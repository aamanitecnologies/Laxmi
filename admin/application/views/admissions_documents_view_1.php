                                        ...
                                        <div class="signature-pad--footer">
                                            ...
                                            <div class="signature-pad--actions">
                                                ...
                                                <div>
                                                    ...
                                                    <input type="file" name="signature" id="signature-b">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="upload-signature-block text-xs-center">
                                        <div class="profile-avatar" id="dvPreview-signature">

                                        </div>
                                    <!--img src="<?php echo base_url(); ?>/assets/img/signature.png" class="signature-img"-->
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-6">
                                    ...
                                    <div class="card-block">
                                        <div class="row">
                                            <div class="profile-avatar" id="dvPreview-id-proof">
                                                <img width="200" src="<?php echo base_url(); ?>/assets/img/user.png" alt="">
                                            </div>
...
//                    }
                ...
                $("#id-proof").change(function () {
                    $("#dvPreview-id-proof").html("");
                    var regex = /^([a-zA-Z0-9\s_\\.\-:])+(.jpg|.jpeg|.gif|.png|.bmp)$/;
                    if (regex.test($(this).val().toLowerCase())) {
                        if ($.browser.msie && parseFloat(jQuery.browser.version) <= 9.0) {
                            $("#dvPreview-id-proof").show();
                            $("#dvPreview-id-proof")[0].filters.item("DXImageTransform.Microsoft.AlphaImageLoader").src = $(this).val();
                        } else {
                            if (typeof (FileReader) != "undefined") {
                                $("#dvPreview-id-proof").show();
                                $("#dvPreview-id-proof").append("<img />");
                                var reader = new FileReader();
                                reader.onload = function (e) {
                                    $("#dvPreview-id-proof img").attr("src", e.target.result);
                                    $("#dvPreview-id-proof img").attr("width", 200);
                                }
                                reader.readAsDataURL($(this)[0].files[0]);
                            } else {
                                alert("This browser does not support FileReader.");
                ...
                $("#signature-b").change(function () {
                    $("#dvPreview-signature").html("");
                    var regex = /^([a-zA-Z0-9\s_\\.\-:])+(.jpg|.jpeg|.gif|.png|.bmp)$/;
                    if (regex.test($(this).val().toLowerCase())) {
                        if ($.browser.msie && parseFloat(jQuery.browser.version) <= 9.0) {
                            $("#dvPreview-signature").show();
                            $("#dvPreview-signature")[0].filters.item("DXImageTransform.Microsoft.AlphaImageLoader").src = $(this).val();
                        } else {
                            if (typeof (FileReader) != "undefined") {
                                $("#dvPreview-signature").show();
                                $("#dvPreview-signature").append("<img />");
                                var reader = new FileReader();
                                reader.onload = function (e) {
                                    $("#dvPreview-signature img").attr("src", e.target.result);
                                    $("#dvPreview-signature img").attr("width", 200);
                                }
                                reader.readAsDataURL($(this)[0].files[0]);
                            } else {
                                alert("This browser does not support FileReader.");
